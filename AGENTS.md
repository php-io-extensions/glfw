...
6. Build: Zephir sources in `glfw/glfw/**/*.zep`; committed C under `ext/`. Installers: `install-macos.sh`, `install-macos-herd.sh`, `install-debian-trixie.sh`, `install-jetpack6.sh`. PIE: `pie install php-io-extensions/glfw`. Version target: **0.7.1**.
