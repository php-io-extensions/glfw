...
# Debian Trixie / Raspberry Pi OS installer for the glfw PHP extension.
#
...
header() {
    echo "============================================"
    echo " GLFW Extension Installer (Debian / Pi OS) "
    echo "============================================"
    echo ""
}
...
# ---------------------------------------------------------------------------
# Dependency: GLFW 3.4+ headers/libs
# Prefer apt libglfw3-dev; fall back to building GLFW 3.4 from source.
# ---------------------------------------------------------------------------
...
ensure_glfw() {
    ...
    if command -v pkg-config >/dev/null 2>&1 && pkg-config --exists glfw3 2>/dev/null; then
        local installed_ver
        installed_ver="$(pkg-config --modversion glfw3)"
        if version_ge "$installed_ver" "$MIN_GLFW_VERSION"; then
            ok "GLFW ${installed_ver} detected via pkg-config (meets minimum)"
            return
        fi
        echo "   ⚠️  GLFW ${installed_ver} is installed but < ${MIN_GLFW_VERSION}; building a newer version from source."
        build_glfw_from_source
    else
        step "GLFW not detected — trying apt first..."
        require_cmd apt-get
        $SUDO apt-get update -q

        if $SUDO apt-get install -y --no-install-recommends pkg-config libglfw3-dev 2>/dev/null; then
            local apt_ver
            apt_ver="$(pkg-config --modversion glfw3 2>/dev/null || echo '0.0.0')"
            if version_ge "$apt_ver" "$MIN_GLFW_VERSION"; then
                ok "GLFW ${apt_ver} installed from apt (meets minimum)"
                return
            fi
            echo "   ⚠️  apt provided GLFW ${apt_ver} which is < ${MIN_GLFW_VERSION}; building from source on top."
            build_glfw_from_source
        else
            step "libglfw3-dev not in apt — going straight to source build..."
    ...
    if ! pkg-config --exists glfw3 2>/dev/null; then
        die "GLFW still not detected by pkg-config. Check /usr/local/lib/pkgconfig."
    fi

    local final_ver
    final_ver="$(pkg-config --modversion glfw3)"
    ok "GLFW ${final_ver} ready"
}
...
# Carry source-built GLFW prefix into configure so pkg-config finds the right version.
export PKG_CONFIG_PATH="/usr/local/lib/pkgconfig:${PKG_CONFIG_PATH:-}"
